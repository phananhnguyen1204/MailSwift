...
function getEmailData() {
  ...
  emailData.subject = subjectContainer
    ? subjectContainer.innerText
    : "Subject not found";

  // Extract the sender's email address
  const senderContainer = document.querySelector("span.gD");
  const senderName = senderContainer
    ? senderContainer.innerText
    : "Sender name not found";
  const senderEmail = senderContainer
    ? senderContainer.getAttribute("email")
  ...
  emailData.timestamp = timestampContainer
    ? timestampContainer.getAttribute("title")
    : "Timestamp not found";
...
// Function to send the message with the email data to the background script
function sendEmailDataToBackground() {
...
function tryToSendMessage(emailData, retries = 3) {
  try {
    if (chrome.runtime && chrome.runtime.sendMessage) {
      ...
      chrome.runtime.sendMessage(
        { action: "generateEmailResponse", emailData: emailData },
        (response) => {
      ...
      throw new Error(
        "chrome.runtime or chrome.runtime.sendMessage is not available"
      );
    }
  } catch (e) {
    if (retries > 0) {
      console.error(
        `Retrying sendMessage due to error: ${e.message}. Retries left: ${
          retries - 1
        }`
      );
...
// Listen for changes in the Gmail page
const observer = new MutationObserver(function (mutations) {
  mutations.forEach((mutation) => {
    if (mutation.type === "childList" && mutation.addedNodes.length) {
      sendEmailDataToBackground();
    }
  });
});
